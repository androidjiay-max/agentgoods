...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "signup") {
      ...
      setSuccess("Account created! Check your email for a confirmation link, or sign in now if auto-confirmed.")
      setMode("signin")
      setPassword("")
      setLoading(false)
      return
    ...
    <main className="min-h-screen bg-dark-bg flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-gray-500 mt-1">M2M Protocol Gateway</p>
        </div>
            ...
            {(["signin", "signup"] as Mode[]).map((m) => (
              <button
                ...
                className={`flex-1 py-1.5 rounded-md text-xs font-medium transition-all ${
                  mode === m ? "bg-neon-blue/15 text-white" : "text-gray-500 hover:text-gray-300"
                }`}
              >
